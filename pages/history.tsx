...
import { faBuilding, faEllipsisVertical, faPlay } from '@fortawesome/free-solid-svg-icons'
...
const History: NextPage = () => {
  ...
  //Get actual date without hour
  let actualDateNoHour = `${current.getDate()}/${current.getMonth() + 1}/${current.getFullYear()}`

  //Get one week before date without hour
  let oneWeekBeforeDateNoHour = `${current.getDate() - 7}/${
    current.getMonth() + 1
  }/${current.getFullYear()}`

  //Check if the is not setted the date of begin search
  function dataBeginCheck() {
    if (dataBegin === '') {
      //Set the date of begin in the format YYYYMMDD
      setDataBegin(oneWeekBeforeDateNoHour.split('/').reverse().join(''))
      return
    } else {
      //Set the date of begin in the format YYYYMMDD
  ...
  //Check if the is not setted the date of end search
  function dataEndCheck() {
    if (dataEnd === '') {
      setDataEnd(actualDateNoHour.split('/').reverse().join(''))
      return
    } else {
  ...
  function checkTypeSource(contact: any) {
    ...
    if (contactType === 'user') {
      return (
        <div className='text-gray-900 truncate cursor-pointer no-underline hover:underline'>Me</div>
      )
    } else {
      //Switchboard call type
      return (
        <>
          <div className='text-gray-900 truncate cursor-pointer no-underline hover:underline'>
            {contact.cnam !== ''
              ? contact.cnam
              : contact.ccompany !== ''
              ? contact.ccompany
          ...
          {contact.cnum !== '' && (
            <div className='text-gray-500 truncate cursor-pointer no-underline hover:underline'>
              {contact.src}
            </div>
          )}
        </>
  ...
  function checkTypeDestination(contact: any) {
    ...
    if (contactType === 'user') {
      ...
      return (
        <>
          <div className='text-gray-900 cursor-pointer no-underline hover:underline'>
            {contact.dst_cnam !== ''
              ? contact.dst_cnam
              : contact.dst_ccompany !== ''
              ? contact.dst_ccompany
          ...
          {(contact.dst_cnam !== '' || contact.dst_ccompany !== '') && (
            <div className='truncate text-gray-500 cursor-pointer no-underline hover:underline'>
              {contact.dst}
            </div>
          )}
        </>
                      ...
                      history.rows.map((contact: any, index: number) => (
                        <tr key={index}>
                          ...
                          <td
                            ...
                            onClick={() => openDrawerHistory(contact, "source")}
                          >
                            {checkTypeSource(contact)}
                          </td>

                          {/* Destination column  */}
                          <td
                            className='whitespace-nowrap px-3 py-4 text-sm text-gray-500'
                            onClick={() => openDrawerHistory(contact, "destination")}
                          >
