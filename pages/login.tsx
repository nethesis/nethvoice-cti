...
import { saveCredentials, updateAuthStore } from '../lib/login'
...
import { axiosInit, updateAxiosToken } from '../config/axios'
import {
...
export default function Login() {
  ...
  const handleLogin = async (callStatus: any, token: any, username: any) => {
    console.log('handleLogin called with:', callStatus, token, username)
    if (token) {
  ...
  const verifyOTP = async (e: React.FormEvent) => {
    ...
    if (!tempLoginData || !otpCode || otpCode.length !== 6) {
      setOnError(true)
      setMessaggeError(t('Login.Invalid verification code') || 'Invalid verification code')
      return
    }
    ...
    try {
      ...
      if (response?.data?.token) {
        ...
        updateAuthStore(tempLoginData.username, response.data.token)

        handleLogin(200, response.data.token, tempLoginData.username)
      }
    } catch (error) {
      setOnError(true)
      setMessaggeError(t('Login.Invalid verification code') || 'Invalid verification code')
    ...
    return (
      <div className='max-w-sm sm:w-96'>
        <div className='flex flex-col items-center justify-center'>
          {/* Logo */}
          <div className='relative w-auto h-16 items-center'>
            <Image
              className='object-contain object-bottom fill-current text-primary dark:text-primaryDark'
        ...
        <div className='pt-6 text-center'>
          ...
          <p className='text-sm text-gray-600 dark:text-gray-400 mb-6'>
            {t('Login.Enter the 6-digit code from your authenticator app')}
          </p>
        </div>
        ...
        <div className='pt-6'>
          <form onSubmit={verifyOTP} className='space-y-6'>
            ...
            <div className='space-y-4'>
              ...
              >
                <span className='font-medium leading-5 text-sm'>{t('Login.Verify')}</span>
              ...
              >
                ...
                <span className='font-medium leading-5 text-sm'>{t('Login.Back to login')}</span>
              </Button>
            </div>
          </form>
        </div>
