...
export interface SummaryCallInfo {
  ...
  // Call sentiment score (1-10) generated by AI: 1-3 negative, 4-6 neutral, 7-10 positive.
  sentiment?: number
}

type SentimentLevel = 'negative' | 'neutral' | 'positive'

const getSentimentLevel = (score?: number): SentimentLevel | undefined => {
...
const sentimentConfig: Record<
  SentimentLevel,
  { label: string; variant: StatusTypes; icon: IconDefinition }
> = {
  negative: {
...
export const CallInformationSection: FC<CallInformationSectionProps> = ({ callInfo, isLoading }) => {
  const sentimentLevel = getSentimentLevel(callInfo?.sentiment)
  const sentiment = sentimentLevel ? sentimentConfig[sentimentLevel] : undefined
  const sentimentTooltipId = 'summary-sentiment-tooltip'
    ...
    <div className='mt-6 flex flex-col gap-4'>
      ...
      {!isLoading && sentiment && (
        <div className='flex items-center gap-8 w-full'>
          ...
          <span
            data-tooltip-id={sentimentTooltipId}
            data-tooltip-content={
              t('Summary.Sentiment score tooltip', { score: callInfo?.sentiment }) || ''
            }
            className='w-fit'
          >
            <Badge
              ...
              rounded='full'
              size='large'
              icon={<FontAwesomeIcon icon={sentiment.icon} className='h-3.5 w-3.5' />}
            >
              {t(sentiment.label)}
            </Badge>
          </span>
          <CustomThemedTooltip id={sentimentTooltipId} place='top' />
        </div>
      )}
    </div>
  )
