...
export const Filter = forwardRef<HTMLButtonElement, FilterProps>(
  (
    ...
    const dateToWithHour = moment().format('YYYY/MM/DD HH:mm')
    const dateFromWithHour = moment().startOf('day').subtract(7, 'd').format('YYYY/MM/DD HH:mm')

    //Set the date of begin search
    const changeDateBegin = () => {
      //get the date from the input
      const dateBegin = dateBeginRef.current.value
      let dateBeginWithHour = moment(dateBegin).format('YYYY/MM/DD HH:mm')
      //Get the date without hour
      let noHour = moment(dateBegin).format('YYYY-MM-DD')
      // update history (notify parent component)
    ...
    const changeDateEnd = () => {
      ...
      let dateEndWithHour = moment(dateEnd).format('YYYY/MM/DD HH:mm')
      //Get the date without hour
      let noEndHour = moment(dateEnd).format('YYYY-MM-DD')
      // update history (notify parent component)
    ...
    useEffect(() => {
      ...
      if (
        contactTypeFound &&
        contactTypeFound.label === 'Personal' &&
        contactDirection === 'internal'
      ) {
    ...
    function clearFilters() {
      ...
      setContactType('user')
      setContactDirection('all')
      setdateBeginValue(dateBeginValue)
      setdateEndValue(dateEndValue)
      setSortBy('time%20asc')
      updateFilterText('')
      updateContactTypeFilter('user')
      updateContactDirectionFilter('all')
      updateDataBeginFilter('')
      updateDataEndFilter('')
      updateSortFilter('time%20asc')
    }
                        ...
                        <Popover.Panel className='absolute right-0 z-10 mt-2 origin-top-right rounded-md flex flex-col space-y-4 bg-white p-4 shadow-2xl ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-gray-900 dark:ring-gray-700 '>
                          ...
                          <div className='relative '>
                            <div className='absolute inset-0 flex items-center' aria-hidden='true'>
                              <div className='w-full border-t  bg-gray-300 dark:bg-gray-600' />
                            </div>
                          </div>
              ...
              <div>
                <div className='mx-auto pt-3 sm:flex sm:items-center'>
                  ...
                  <div className='mt-2 sm:mt-0 sm:ml-4'>
                    <div className='-m-1 flex flex-wrap items-center'>
                      <span className='m-1 inline-flex items-center rounded-full border py-1.5 pl-3 pr-2 text-sm font-medium border-gray-200 bg-white text-gray-700 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200'>
                        ...
                        {!dateBeginValue ? dateFromWithHour : dateBeginShowed}
                      </span>
                    </div>
                  </div>
                  <div className='mt-2 sm:mt-0 sm:ml-4'>
                    <div className='-m-1 flex flex-wrap items-center'>
                      <span className='m-1 inline-flex items-center rounded-full border py-1.5 pl-3 pr-2 text-sm font-medium border-gray-200 bg-white text-gray-700 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200'>
                        ...
                        {!dateEndValue ? dateToWithHour : dateEndShowed}
                      </span>
                    </div>
                  </div>
                  <div
                  ...
                  <div className='pl-6'>
                    <button
                      type='button'
                      className='text-gray-500 dark:text-gray-400 no-underline hover:underline'
